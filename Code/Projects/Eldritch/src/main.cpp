...
#if BUILD_MAC
	// TODO use proper FIleUtil:: routine
...
#if BUILD_MAC
	LogFilename = ObjCJunk::GetUserDirectory() + SimpleString( "meldritch-log.txt" );
...
#if BUILD_DEBUG
	SETPRINTCHANNELS( PRINTCHANNEL_Console | PRINTCHANNEL_Output | PRINTCHANNEL_Log );
#else
	SETPRINTCHANNELS( PRINTCHANNEL_Log );
#endif
}
...
#if BUILD_DEBUG
		if ( !Allocator::GetDefault().CheckForLeaks() )
		{
			Allocator::GetDefault().Report( PrintManagerStream() );
		}
#endif
