...
void EldritchFramework::InitializeMods()
{
	ModManager::GetInstance()->AddMod( ( GetSaveLoadPath() + "Mods/mEldritch/" ).CStr() );
	ModManager::GetInstance()->InitializeMods( ( GetSaveLoadPath() + "Mods/mods.cfg" ).CStr() );
}
...
{
	// TODO also load from Mods, User Home, ...
...
{
	// TODO write back to where we loaded it from
	const SimpleString PrefsConfigFilename = GetSaveLoadPath() + SimpleString( "prefs.cfg" );
	...
	m_Tools->WriteConfig( PrefsConfigStream );
}
...
/* enable when needed
	...
	{
		STATIC_HASHED_STRING( SelectModScreen );
		WB_MAKE_EVENT( PushUIScreen, NULL );
		WB_SET_AUTO( PushUIScreen, Hash, Screen, sSelectModScreen );
		WB_DISPATCH_EVENT( WBWorld::GetInstance()->GetEventManager(), PushUIScreen, NULL );
	}
#endif
//*/
...
#else
	{
		if( m_Keyboard->OnRise( Keyboard::EB_F5 ) )	// Quicksave
		{
			m_Game->GetSaveLoad()->SaveMaster( "quick.meldritchmastersave" );
		}

		if( m_Keyboard->OnRise( Keyboard::EB_F6 ) )	// Quickload
		{
			PrepareForLoad();
			if ( m_Game->GetSaveLoad()->TryLoadMaster( "quick.meldritchmastersave" ) )
			{
				m_Game->GetPersistence()->SetCheater( true );
				ShowIsCheater( true );
			}
		}

#if  DEBUG && BUILD_WINDOWS_NO_SDL
		// Alt + S saves
		if( m_Keyboard->IsHigh( Keyboard::EB_LeftAlt ) && m_Keyboard->OnRise( Keyboard::EB_S ) )
		{
			SimpleString SaveFileName;
			if( FileUtil::GetSaveFile( GetWindow()->GetHWnd(), "Save Files", "meldritchmastersave", SaveFileName ) )
			{
		...
		if( m_Keyboard->IsHigh( Keyboard::EB_LeftAlt ) && m_Keyboard->OnRise( Keyboard::EB_L ) )
		{
			...
			if( FileUtil::GetLoadFile( GetWindow()->GetHWnd(), "Save Files", "meldritchmastersave", LoadFileName ) )
			{
				PrepareForLoad();
	...
	if( m_Keyboard->OnRise( Keyboard::EB_H ) )
	{
		WBWorld *World = WBWorld::GetInstance();
		WBEventManager* WorldEventManager = World->GetEventManager();

		STATIC_HASHED_STRING( HUD );
		STATIC_HASHED_STRING( EmptyScreen );

		UIManager* const	pUIManager	= GetUIManager();
		UIScreen* const		pHUD		= pUIManager->GetScreen( sHUD );
		UIScreen* const		pEmpty		= pUIManager->GetScreen( sEmptyScreen );
		...
		if ( m_Keyboard->IsHigh( Keyboard::EB_LeftControl ) )
		{
			...
			if ( m_Keyboard->IsHigh( Keyboard::EB_LeftShift ) )
			{
				WB_MAKE_EVENT( HideHands, m_Game->GetPlayer() );
				WB_DISPATCH_EVENT( WorldEventManager, HideHands, m_Game->GetPlayer() );
			}
			pUIManager->GetUIStack()->Replace( pHUD, pEmpty );
		}
		else if ( m_Keyboard->IsHigh( Keyboard::EB_LeftShift ) )
		{
			// Show HUD and stuff
			WB_MAKE_EVENT( ShowHands, m_Game->GetPlayer() );
			WB_DISPATCH_EVENT( WorldEventManager, ShowHands, m_Game->GetPlayer() );
			pUIManager->GetUIStack()->Replace( pEmpty, pHUD );
		}
	}
...
/*virtual*/ void EldritchFramework::TickRender()
{
	...
	m_Renderer->SetRenderTarget( m_TargetManager->GetPrimaryRenderTarget() ); // TODO ?
}
...
void EldritchFramework::ShowIsCheater( bool IsCheater )
{
	UIManager* const			pUIManager		= GetUIManager();
	ASSERT( pUIManager );
	...
	STATIC_HASHED_STRING( CheaterMarker );
	UIWidgetImage* const		pCheaterMarker	= pHUDScreen->GetWidget<UIWidgetImage>( sCheaterMarker );
	if( !pCheaterMarker )
	{
		return;
	}

	pCheaterMarker->SetHidden( !IsCheater );
}
