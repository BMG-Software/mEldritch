...
void EldritchTools::ShutDown()
{
	...
	const uint NumVoxelSetButtons = m_VoxelSetButtons.Size();
	for( uint VoxelSetButtonIndex = 0; VoxelSetButtonIndex < NumVoxelSetButtons; ++VoxelSetButtonIndex )
	{
		SVoxelSetButton& VoxelSetButton = m_VoxelSetButtons[ VoxelSetButtonIndex ];
		SafeDelete( VoxelSetButton.m_Mesh );
	}
	m_VoxelSetButtons.Clear();

	SafeDelete( m_HelpMesh );
	SafeDelete( m_AltHelpMesh );
}

const EldritchWorld::SWorldDef& EldritchTools::GetWorldDef() const
{
	HashedString WorldDef = m_VoxelSets[ m_CurrentVoxelSet ].m_WorldDef;
	DEBUGASSERT( m_World->m_WorldDefs.Search( WorldDef ).IsValid() );
	return m_World->m_WorldDefs[ WorldDef ];
}
...
void EldritchTools::InitializeFromDefinition( const SimpleString& DefinitionName )
{
	...
	STATICHASH( Font );
	const char* TheFont = ConfigManager::GetString( sFont, NULL, sDefinitionName );
	if ( TheFont )
	{
		m_Font = pRenderer->GetFontManager()->GetFont( TheFont );
	}
	if ( !m_Font )
	{
		m_Font = pRenderer->GetFontManager()->GetFont( DEFAULT_FONT );
	}
...
void EldritchTools::Reinitialize( const uint RoomScalarX /*= 1*/, const uint RoomScalarY /*= 1*/, const uint RoomScalarZ /*= 1*/ )
{
	...
	// TODO should use first world def or m_World def per default
	InitializeVoxelSets();
	SelectVoxelSet( m_World->m_CurrentWorldDef );
...
EldritchWorld::SVoxelDef EldritchTools::sFallbackVoxelDef;

void EldritchTools::InitializeVoxelSets()
{
	static const float kBase	= 16.0f;
	static const float kSpacing	= 24.0f;

	STATICHASH( VoxelSets );
	STATICHASH( NumVoxelSets );

	m_VoxelSets.Clear();

	uint NumVoxelSets = ConfigManager::GetInt( sNumVoxelSets, 0, sVoxelSets );
	for ( uint VoxelSetIndex = 0; VoxelSetIndex < NumVoxelSets; ++VoxelSetIndex )
	{
		const char* Description = ConfigManager::GetSequenceString( "VoxelSet%dDesc", VoxelSetIndex, "", sVoxelSets );
		const char* WorldDef = ConfigManager::GetSequenceString( "VoxelSet%dWorld", VoxelSetIndex, "", sVoxelSets );
		m_VoxelSets.PushBack( SVoxelSet( Description, WorldDef ) );
		PRINTF("Using Voxel Set from world \"%s\" (%s)\n", Description, WorldDef );
	}
	m_CurrentVoxelSet = 0;
	...
	for( uint VoxelSetIndex = 0; VoxelSetIndex < NumVoxelSets; ++VoxelSetIndex )
	{
		const SimpleString& VoxelSet = m_VoxelSets[ VoxelSetIndex ].m_Description;
		SRect Rect = SRect( kBase, kBase + VoxelSetIndex * kSpacing, 0.0f, 0.0f );
		Mesh* const pVoxelSetMesh = pRenderer->Print( VoxelSet.CStr(), m_Font, Rect, 0 );

		SVoxelSetButton VoxelSetButton;
		VoxelSetButton.m_Mesh			= pVoxelSetMesh;
		VoxelSetButton.m_WorldDef		= m_VoxelSets[ VoxelSetIndex ].m_WorldDef;
		VoxelSetButton.m_BoxMin			= Vector2( pVoxelSetMesh->m_AABB.m_Min.x, pVoxelSetMesh->m_AABB.m_Min.z );
		VoxelSetButton.m_BoxMax			= Vector2( pVoxelSetMesh->m_AABB.m_Max.x, pVoxelSetMesh->m_AABB.m_Max.z );

		m_VoxelSetButtons.PushBack( VoxelSetButton );
	}
}

void EldritchTools::SelectVoxelSet( HashedString WorldDef )
{
	DEBUGASSERT( m_World->m_WorldDefs.Search( WorldDef ).IsValid() );
	HashedString Extends = m_World->m_WorldDefs[ WorldDef ].m_Extends;

	for ( uint VoxelSetIter = 0; VoxelSetIter < m_VoxelSets.Size(); ++VoxelSetIter )
	{
		HashedString& IterWorldDef = m_VoxelSets[ VoxelSetIter ].m_WorldDef;
		DEBUGASSERT( m_World->m_WorldDefs.Search( IterWorldDef ).IsValid() );
		HashedString& IterExtends = m_World->m_WorldDefs[ IterWorldDef ].m_Extends;

		// this is our idea of equivalence between 2 WorldDefs, for the purpose of this function
		if( IterWorldDef == WorldDef || IterWorldDef == Extends || IterExtends == WorldDef || IterExtends == Extends )
		{
			SelectVoxelSet( VoxelSetIter );
			return;
		}
	}
	SelectVoxelSet( 0 );
...
void EldritchTools::SelectVoxelSet( uint VoxelSet )
{
	if ( VoxelSet >= 0 && VoxelSet < m_VoxelSets.Size() )
	{
		m_CurrentVoxelSet = VoxelSet;

		STATICHASH( EldritchWorld );
		STATICHASH( TileSetSizeX );
		STATICHASH( TileSetSizeY );
		m_NumTilesX		= ConfigManager::GetInt( sTileSetSizeX, 0, sEldritchWorld );
		m_NumTilesY		= ConfigManager::GetInt( sTileSetSizeY, 0, sEldritchWorld );
		...
		ReinitializePaletteMeshes();
		RefreshVoxelMeshes();

		sFallbackVoxelDef.m_SideTile = m_NumTiles - 1;
		sFallbackVoxelDef.m_TopTile = m_NumTiles - 1;
		sFallbackVoxelDef.m_BottomTile = m_NumTiles - 1;
	}
...
// DEBUG?
void EldritchTools::CycleVoxelSets()
{
	if (m_CurrentVoxelSet + 1 < m_VoxelSets.Size())
	{
		SelectVoxelSet( m_CurrentVoxelSet + 1 );
	}
	else
	{
		SelectVoxelSet( 0 );
...
const EldritchWorld::SVoxelDef& EldritchTools::GetVoxelDef( const vval_t Voxel ) const
{
	const EldritchWorld::SWorldDef& CurrentWorldDef = GetWorldDef();

	if( CurrentWorldDef.m_VoxelDefs.Search( Voxel ).IsValid() )
	{
		return CurrentWorldDef.m_VoxelDefs[ Voxel ];
	}
	return sFallbackVoxelDef;
}

void EldritchTools::SetColorGrading()
{
	HashedString TheWorldDef = m_ToolMode ? m_VoxelSets[ m_CurrentVoxelSet ].m_WorldDef : m_World->m_CurrentWorldDef;
	const EldritchWorld::SWorldDef&	CurrentWorldDef	= m_World->m_WorldDefs[ TheWorldDef ];

	EldritchGame* const	pGame = m_Framework->GetGame();
...
void EldritchTools::InitializeHelpMeshes()
{
	...
	const SimpleString HelpText = SimpleString( "F5: Save F6: Load F7: Clear F8: Fill\nQ: Up E: Down\nEnter: Fill Shift+Enter: Erase\nHold Alt for more options." );
	m_HelpMesh = pRenderer->Print( HelpText.CStr(), m_Font, SRect(), 0 );
	...
	m_AltHelpMesh = pRenderer->Print( AltHelpText.CStr(), m_Font, SRect(), 0 );
}
...
void EldritchTools::InitializeSpawnerMeshes()
{
	...
	for( uint CategoryIndex = 0; CategoryIndex < NumCategories; ++CategoryIndex )
	{
		...
		Mesh* const pCategoryMesh = pRenderer->Print( SpawnerCategory.CStr(), m_Font, Rect, 0 );
	...
	for( uint SpawnerDefIndex = 0; SpawnerDefIndex < NumSpawnerDefs; ++SpawnerDefIndex )
	{
		...
		Mesh* const pSpawnerDefMesh = pRenderer->Print( SpawnerDef.CStr(), m_Font, Rect, 0 );
...
void EldritchTools::ReinitializePaletteMeshes()
{
	const uint NumPaletteVoxels = m_PaletteVoxels.Size();
	for( uint PaletteVoxelIndex = 0; PaletteVoxelIndex < NumPaletteVoxels; ++PaletteVoxelIndex )
	{
...
{
	// TODO
	const Map<vval_t, EldritchWorld::SVoxelDef>& VoxelDefs = GetWorldDef().m_VoxelDefs;
...
void EldritchTools::CreatePaletteMesh( const vval_t VoxelValue, const EldritchWorld::SVoxelDef& VoxelDef )
{
	...
	// static const float	kCos60			= 0.5f; // unused
...
#undef PUSH_POSITIONS
	...
	const Vector2		UVBaseTop				= m_TileUVsMap[ VoxelDef.m_TopTile ];
	const Vector2		UVBaseSide				= m_TileUVsMap[ VoxelDef.m_SideTile ];
	const float			TileU					= m_TileU;
	const float			TileV					= m_TileV;
...
void EldritchTools::CreateVoxelMesh( const vidx_t VoxelIndex )
{
	...
	const EldritchWorld::SVoxelDef& VoxelDef	= GetVoxelDef( VoxelValue );
...
void EldritchTools::Tick( const float DeltaTime )
{
	...
	// static const float kVelocity			= 2.0f; // unused
...
void EldritchTools::TickInput()
{
	...
	{
		SetSubTool( m_SubTool != EST_VoxelSets ? EST_VoxelSets : EST_None );
	}

	if( pKeyboard->OnRise( Keyboard::EB_F3 ) )
	{
		SetSubTool( m_SubTool != EST_Palette ? EST_Palette : EST_None );
	}

	if( pKeyboard->OnRise( Keyboard::EB_F4 ) )
	{
		SetSubTool( m_SubTool != EST_Spawners ? EST_Spawners : EST_None );
	}

	if( m_SubTool == EST_VoxelSets )
	{
		TickVoxelSetsInput();
	}
	else if( m_SubTool == EST_Palette )
	{
		TickPaletteInput();
	}
...
void EldritchTools::TickVoxelSetsInput()
{
	Keyboard* const pKeyboard = m_Framework->GetKeyboard();
	Mouse* const pMouse = m_Framework->GetMouse();
	...
	{
		const uint NumVoxelSetButtons = m_VoxelSetButtons.Size();
		for( uint VoxelSetIndex = 0; VoxelSetIndex < NumVoxelSetButtons; ++VoxelSetIndex )
		{
			const SVoxelSetButton& VoxelSetButton = m_VoxelSetButtons[ VoxelSetIndex ];
			...
			if( fX >= VoxelSetButton.m_BoxMin.x &&
				fY >= VoxelSetButton.m_BoxMin.y &&
				fX <= VoxelSetButton.m_BoxMax.x &&
				fY <= VoxelSetButton.m_BoxMax.y )
			{
				// Clicked
				SelectVoxelSet( VoxelSetIndex );
				SetSubTool( EST_None );
			}
		}
	}
...
void EldritchTools::SetSubTool( const ESubTool SubTool )
{
	...
	if( m_SubTool == EST_VoxelSets || m_SubTool == EST_Palette || m_SubTool == EST_Spawners )
	{
...
void EldritchTools::TickNormalInput()
{
	...
	// TODO clashes with screenshots?
	if( pKeyboard->OnRise( Keyboard::EB_S ) && pKeyboard->IsHigh( Keyboard::EB_LeftControl ) && !pKeyboard->IsHigh( Keyboard::EB_LeftShift ) )
	{
		TryQuickSave();
	}

	if( pKeyboard->OnRise( Keyboard::EB_F5 ) )
	{
		TrySave();
	}
...
void EldritchTools::TickBrushInput()
{
	...
	{
		if( pKeyboard->IsHigh( Keyboard::EB_LeftShift ) )			{ AdjustBrush( 0, -1, 1 ); }	// Expand brush west
		else if( pKeyboard->IsHigh( Keyboard::EB_LeftControl ) )	{ AdjustBrush( 0, 0, -1 ); }	// Shrink brush west
		else													{ AdjustBrush( 0, -1, 0 ); }		// Move brush west
	}

	if( pKeyboard->OnRise( Keyboard::EB_L ) )
	{
		if( pKeyboard->IsHigh( Keyboard::EB_LeftShift ) )			{ AdjustBrush( 0, 0, 1 ); }		// Expand brush east
		else if( pKeyboard->IsHigh( Keyboard::EB_LeftControl ) )	{ AdjustBrush( 0, 1, -1 ); }	// Shrink brush east
		else													{ AdjustBrush( 0, 1, 0 ); }			// Move brush east
	}

	if( pKeyboard->OnRise( Keyboard::EB_I ) )
	{
		if( pKeyboard->IsHigh( Keyboard::EB_LeftShift ) )			{ AdjustBrush( 1, 0, 1 ); }		// Expand brush north
		else if( pKeyboard->IsHigh( Keyboard::EB_LeftControl ) )	{ AdjustBrush( 1, 1, -1 ); }	// Shrink brush north
		else													{ AdjustBrush( 1, 1, 0 ); }			// Move brush north
	}

	if( pKeyboard->OnRise( Keyboard::EB_O ) )
	{
		if( pKeyboard->IsHigh( Keyboard::EB_LeftShift ) )			{ AdjustBrush( 1, -1, 1 ); }	// Expand brush south
		else if( pKeyboard->IsHigh( Keyboard::EB_LeftControl ) )	{ AdjustBrush( 1, 0, -1 ); }	// Shrink brush south
		else													{ AdjustBrush( 1, -1, 0 ); }		// Move brush south
	}
...
/* static */ void EldritchTools::DoClear( void *pUIElement, void* pVoid )
{
	EldritchTools *pEldritchTools = static_cast<EldritchTools*>( pVoid );
	const bool WithFloor = pEldritchTools->m_ClearInfo.WithFloor;
	const bool WithShell = pEldritchTools->m_ClearInfo.WithShell;
	pEldritchTools->Clear( WithFloor, WithShell );
}
void EldritchTools::TryClear( const bool WithFloor, const bool WithShell )
{
	m_ClearInfo.WithFloor = WithFloor;
	m_ClearInfo.WithShell = WithShell;
	m_Framework->GetUIManager()->ShowYesNoDialog( true, "Clear World?", "Clear World?", "Pop", "Pop", "", "", SUICallback( DoClear, this ) );
}
...
/* static */ void EldritchTools::DoFill( void *pUIElement, void* pVoid )
{
	Unused(pUIElement);
	EldritchTools *pEldritchTools = static_cast<EldritchTools*>(pVoid);
	pEldritchTools->Fill();
...
{
	m_Framework->GetUIManager()->ShowYesNoDialog( true, "Fill World?", "Fill World?", "Pop", "Pop", "", "", SUICallback( DoFill, this ) );
}
...
void EldritchTools::TickRender()
{
	...
	{
		// Draw VoxelSets
		const uint NumVoxsetButtons = m_VoxelSets.Size();
		for( uint VoxsetButtonIndex = 0; VoxsetButtonIndex < NumVoxsetButtons; ++VoxsetButtonIndex )
		{
			const SVoxelSetButton& VoxelSetButton = m_VoxelSetButtons[ VoxsetButtonIndex ];
			pRenderer->AddMesh( VoxelSetButton.m_Mesh );

			// Draw a box around the selected category
			if( m_CurrentVoxelSet == VoxsetButtonIndex )
			{
				const Vector BoxMin( VoxelSetButton.m_BoxMin.x, 0.0f, VoxelSetButton.m_BoxMin.y );
				const Vector BoxMax( VoxelSetButton.m_BoxMax.x, 0.0f, VoxelSetButton.m_BoxMax.y );
...
/* static */ void EldritchTools::DoQuickSave( void *pUIElement, void* pVoid )
{
	EldritchTools *pEldritchTools = static_cast<EldritchTools*>(pVoid);
	pEldritchTools->Save( FileStream( pEldritchTools->m_CurrentMapName.CStr(), FileStream::EFM_Write ) );
}
void EldritchTools::TryQuickSave()
{
	...
	m_Framework->GetUIManager()->ShowYesNoDialog( true, Message, Message, "Pop", "Pop", "", "", SUICallback( DoQuickSave, this ) );
}

void EldritchTools::SetLastUsedPath( SimpleString FileName )
{
	STATICHASH( LastUsedPath );
	STATICHASH( EldritchTools );
	if ( FileName == "" )
	{
		m_LastUsedPath = ConfigManager::GetString( sLastUsedPath, "", sEldritchTools );
		if ( m_LastUsedPath == "" || !FileUtil::IsDir( m_LastUsedPath.CStr() ) )
		{
			m_LastUsedPath = FileUtil::GetUserPersonalPath();
		}
	}
	else
	{
		m_LastUsedPath = FileUtil::StripFilename( FileName.CStr() );
	}
	ConfigManager::SetString( sLastUsedPath, m_LastUsedPath.CStr(), sEldritchTools );
}

void EldritchTools::DoSave( void* pName, void* pData )
{
	SimpleString SaveFileName = static_cast< const char* >( pName );
	EldritchTools *pEldritchTools = static_cast<EldritchTools*>(pData);

	if ( !FileUtil::HasExtension( SaveFileName.CStr(), "eldritchroom" ) )
	{
		SaveFileName = SaveFileName + ".eldritchroom";
	}

	pEldritchTools->Save( FileStream( SaveFileName.CStr(), FileStream::EFM_Write ) );
	pEldritchTools->m_CurrentMapName = SaveFileName;
	pEldritchTools->SetLastUsedPath( SaveFileName );
}

void EldritchTools::TrySave()
{
	SimpleString PathName = m_LastUsedPath;
	if ( m_CurrentMapName != "" )
	{
		PathName = m_CurrentMapName;
	}
	m_Framework->GetUIManager()->ShowSelectFileDialog( true, "SaveFile", "SaveFile", PathName, "eldritchroom", SUICallback( DoSave, this ) );
}

void EldritchTools::DoLoad( void* pName, void* pData )
{
	SimpleString LoadFileName = static_cast< const char* >( pName );
	EldritchTools *pEldritchTools = static_cast<EldritchTools*>(pData);
	pEldritchTools->Load( FileStream( LoadFileName.CStr(), FileStream::EFM_Read ) );
	pEldritchTools->m_CurrentMapName = LoadFileName;
	pEldritchTools->SetLastUsedPath( LoadFileName );
}
...
void EldritchTools::TryLoad()
{
	...
	if ( m_CurrentMapName != "" )
	{
		PathName = FileUtil::StripFilename( m_CurrentMapName.CStr() );
	}
	m_Framework->GetUIManager()->ShowSelectFileDialog( true, "LoadFile", "LoadFile", PathName, "eldritchroom", SUICallback( DoLoad, this ) );
}
...
void EldritchTools::Save( const IDataStream& Stream )
{
	...
	TempRoom.m_UsedWorldDef	= m_CurrentVoxelSet;
...
void EldritchTools::Load( const IDataStream& Stream )
{
	...
	m_CurrentVoxelSet = TempRoom.m_UsedWorldDef;
	ReinitializePaletteMeshes();
...
void EldritchTools::WriteConfig( const IDataStream& Stream )
{
	ConfigManager::Write( Stream, "LastUsedPath", "EldritchTools" );
}
