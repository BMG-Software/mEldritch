...
int FilePacker::UnpackFile( const char* PackFilename )
{
	if( FileUtil::Exists( PackFilename ) )
	{
		...
		for( uint i = 0; i < m_FileEntries.Size(); ++i )
		{
			...
			FileEntry.m_Filename.Replace('\\', '/'); // "Hack" to be able to unpack Windows packed files on Linux/MacOS
			// TODO Check whether this still works on windows
...
int FilePacker::ListFiles( const char* PackFilename )
{
	if( FileUtil::Exists( PackFilename ) )
	{
		ReadPackFile( FileStream( PackFilename, FileStream::EFM_Read ) );
