...
MIDI::MIDI() : m_Header(), m_Tracks() {}

MIDI::~MIDI() {
  for (uint TrackIndex = 0; TrackIndex < m_Tracks.Size(); ++TrackIndex) {
    for (uint EventIndex = 0; EventIndex < m_Tracks[TrackIndex].m_Events.Size();
         ++EventIndex) {
...
void MIDI::Load(const IDataStream& Stream) {
  ...
  Stream.Read(14, &m_Header);  // sizeof( SMIDIFileHeader ) is rounded to word
                               // size, so we can't use that
...
void MIDI::LoadTrack(const IDataStream& Stream, SMIDITrack& Track) {
  ...
  while ((uint)(Stream.GetPos() - StartPos) < Track.m_Header.m_ChunkSize) {
    ...
    // Followed by an event code byte (which also includes the channel, for MIDI
    // events)
    const uint8 EventCode = Stream.ReadUInt8();
    MIDIEvent* pEvent = NULL;

    if (EventCode < 0x80) {
      // Running status, rewind the stream and create another of the last MIDI
      // event
...
MIDIEvent* MIDI::EventFactory(const uint8 EventCode,
                              const IDataStream& Stream) const {
  ...
  if (MIDIEventType == 0x08) {
    ...
    // For now, skip SysEx events because I don't care (TODO: I'll need to load
    // them to resave them, though)
    ...
    // For now, skip meta events because I don't care (TODO: I'll need to load
    // them to resave them, though)
...
// NOTE: This doesn't currently set up the chunk length, it just assumes it has
// been filled in properly already.
...
void MIDI::SaveVariableLengthValue(const IDataStream& Stream,
                                   uint32 Value) const {
  while (Value > 0x7F) {
