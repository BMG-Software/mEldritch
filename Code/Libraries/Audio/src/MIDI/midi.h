...
class MIDI {
 ...
 private:
  struct SMIDIChunkHeader {
    SMIDIChunkHeader() : m_ChunkID(0), m_ChunkSize(0) {}
  ...
  struct SMIDIFileHeader {
    SMIDIFileHeader()
        : m_Header(), m_FormatType(0), m_NumTracks(0), m_TimeDivision(0) {}

    SMIDIChunkHeader m_Header;  // "MThd", 6
    uint16 m_FormatType;        // 0-2 (0 is everything on a single track, 1 is
                          // separate tracks, 2 is separate asynchronous tracks)
    uint16 m_NumTracks;     // 1-65535
    uint16 m_TimeDivision;  // SMPTE stuff, see docs
  };

  struct SMIDITrack {
    SMIDITrack() : m_Header(), m_Events() {}
  ...
  MIDIEvent* EventFactory(const uint8 EventCode,
