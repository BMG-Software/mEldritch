...
class SoundCommon : public ISound {
 public:
  ...
  virtual float GetBiasedPan(const float Distance) const {
    return Saturate((Distance - m_PanBiasNear) * m_InversePanBiasRange);
  }
  ...
  void SetFalloffRadius(const float FalloffRadius) {
    m_FalloffRadius = FalloffRadius;
  }
  void SetPanBias(const float Near, const float Far) {
    m_PanBiasNear = Near;
    m_InversePanBiasRange = 1.0f / (Far - Near);
  }
  void SetPanPower(const float Power) { m_PanPower = Power; }
  void SetMinimumAttenuation(const float Attenuation) {
    m_MinimumAttenuation = Attenuation;
  }
  void SetRearAttenuation(const float Attenuation) {
    m_RearAttenuation = Attenuation;
  }
  void SetShouldCalcOcclusion(const bool ShouldCalcOcclusion) {
    m_ShouldCalcOcclusion = ShouldCalcOcclusion;
  }
 ...
 private:
  ...
  float m_PanBiasNear;    // Within Near, pan is centered. At Far, pan is actual
                          // pan.
  float m_InversePanBiasRange;
  float m_PanPower;
  float m_MinimumAttenuation;
  float m_RearAttenuation;  // At 1.0, rear attenuation is full. At 0.0, there
                            // is no rear attenuation.
