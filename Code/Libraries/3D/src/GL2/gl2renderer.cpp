...
/*virtual*/ IVertexShader* GL2Renderer::CreateVertexShader( const SimpleString& Filename )
{
	...
	pVertexShader->Initialize( ModManager::OpenStream( Filename.CStr() ) );
...
/*virtual*/ IPixelShader* GL2Renderer::CreatePixelShader( const SimpleString& Filename )
{
	...
	pPixelShader->Initialize( ModManager::OpenStream( Filename.CStr() ) );
	return pPixelShader;
}
...
/*virtual*/ void GL2Renderer::SaveScreenshot( const SimpleString& Filename )
{
	...
	GLint ViewportDimensions[4];                  // Where The Viewport Values Will Be Stored
	glGetIntegerv(GL_VIEWPORT, ViewportDimensions);
	int x = ViewportDimensions[0];
	int y = ViewportDimensions[1];
	int w = ViewportDimensions[2];
	int h = ViewportDimensions[3];
	...
	*/
	...
	for ( int y = 0; y < h / 2; ++y) {
		unsigned char* from = buf + y * w * 3;
		unsigned char* to = buf + ( h - 1 - y ) * w * 3;
		if ( from != to )
		{
			memcpy( row, from, w * 3 );
			memcpy( from, to , w * 3 );
	...
	stbi_write_png( Filename.CStr(), w, h, 3, buf, w*3 );
}
