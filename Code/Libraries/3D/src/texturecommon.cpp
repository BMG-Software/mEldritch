...
void TextureCommon::InitializeFromFile( const char* const Filename, byte*& OutARGBImage )
{
	...
	if( Ext == EXT_DDS )
	{
		CreateTextureFromDDS( ModManager::OpenStream( Filename ) );
		return;
	}
	else if( Ext == EXT_BMP )
	{
		LoadBMP( ModManager::OpenStream( Filename ), Width, Height, OutARGBImage );
	}
	else if( Ext == EXT_TGA )
	{
		LoadTGA( ModManager::OpenStream( Filename ), Width, Height, OutARGBImage );
	}
	else if( Ext == EXT_PNG )
	{
		LoadPNG( ModManager::OpenStream( Filename ), Width, Height, OutARGBImage );
	}
...
void TextureCommon::LoadPNG( const IDataStream& Stream, int& Width, int& Height, byte*& ARGBImage )
{
	uint BufferSize = Stream.Size();
	unsigned char *Buffer = new unsigned char[BufferSize];
	unsigned char *PNGImage = NULL;
	unsigned int DecWidth, DecHeight;

	Stream.Read( BufferSize, Buffer );
	if ( lodepng_decode32(&PNGImage, &DecWidth, &DecHeight, Buffer, BufferSize) == 0 )
	{
		Width = DecWidth;
		Height = DecHeight;
		ARGBImage = new byte[4 * Width * Height];
		unsigned char *From = PNGImage, *To = ARGBImage;
	...
	{
		WARNDESC( "LoadPNG: invalid image encoding." );
	}
	delete[] Buffer;
}
