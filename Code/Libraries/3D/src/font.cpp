...
// TODO implement more alignments
void Font::Align( const SRect& Bounds, uint Flags, Array<STypesetGlyph>& TypesettingData, Vector2& Dimensions ) const
{
	const SFontLocale& Locale = GetCurrentLocale();
	const float	BoundWidth	= Bounds.m_Right - Bounds.m_Left;
	//const bool	Bounded		= ( BoundWidth > 0.0f ) && ( ( Flags & FONT_SINGLE_LINE ) == 0 );

	if ( ( Flags & FONT_PRINT_MASK ) != FONT_PRINT_LEFT )
	{
		uint Align = Flags & FONT_PRINT_MASK;
		uint IsSingleLine = Flags & FONT_SINGLE_LINE;
		if ( Align == FONT_PRINT_RIGHT || ( ( Align == FONT_PRINT_LTHENR ) && IsSingleLine && Dimensions.x > BoundWidth ) )
		{
			if ( IsSingleLine )
			{
				uint FirstIndex = 0;
				FOR_EACH_ARRAY( GlyphIter, TypesettingData, STypesetGlyph)
				{
					FirstIndex = GlyphIter.GetIndex();
					if ( Dimensions.x + Bounds.m_Left - (*GlyphIter).m_Position.x <= BoundWidth + 2)
					{
						break;
					}
				}
				uint Diff = TypesettingData[FirstIndex].m_Position.x - TypesettingData[0].m_Position.x;
				TypesettingData.SliceToRange(FirstIndex, TypesettingData.Size());
				FOR_EACH_ARRAY( GlyphIter, TypesettingData, STypesetGlyph)
				{
					(*GlyphIter).m_Position.x -= Diff;
				}
			}
			else
			{
		...
		else if ( Align == FONT_PRINT_CENTER )
		{
			if ( IsSingleLine )
			{
				//TODO
		...
		// ALIGN == && ( IsSingleLine && Dimensions.x <= BoundWidth || !IsSingleLine)
		// which all is rendered as FONT_PRINT_LEFT with no change, a.k.a stays as is
	}
	else if ( Flags & FONT_SINGLE_LINE ) // then cut line to BoundWidth
	{
		if ( Dimensions.x > BoundWidth )
		{
			uint LastIndex = 0;
			FOR_EACH_ARRAY( GlyphIter, TypesettingData, STypesetGlyph)
			{
				const SFontCharProp& CharProp = GetCharProp( Locale, (*GlyphIter).m_CodePoint );
				LastIndex = GlyphIter.GetIndex();
				if ( (*GlyphIter).m_Position.x - Bounds.m_Left + CharProp.m_Width >= BoundWidth + 2 )
				{
					LastIndex -= LastIndex ? 1 : 0;
					break;
				}
			}
			TypesettingData.SliceToRange(0, LastIndex);
		}
	}
}
...
void Font::Arrange( const SimpleString& StringUTF8, const SRect& Bounds, uint Flags, Array<STypesetGlyph>& OutTypesetting, Vector2& OutDimensions ) const
{
	Array<unicode_t> CodePoints;
	StringUTF8.UTF8ToUnicode( CodePoints );
	return Arrange( CodePoints, Bounds, Flags, OutTypesetting, OutDimensions );
}

void Font::Arrange( const Array<unicode_t>& CodePoints, const SRect& Bounds, uint Flags, Array<STypesetGlyph>& OutTypesetting, Vector2& OutDimensions ) const
{
	...
	while( EWR_None != ( WordResult = GetNextWord( CodePoints, Locale, Index, WordBuffer, WordWidth ) ) )
	{
		...
		{
			if ( Flags & FONT_SINGLE_LINE )
			{
				// in single line mode we're done here.
				break;
			}
			else
			{
		...
		else
		{
			...
			if( Bounded &&							// Are we doing a bounded print?
				...
				!( Flags & FONT_NOWRAP ) )			// are we using word wrap?
			{
				Cursor.x = 0.0f;
				Cursor.y += m_Height;
			...
			FOR_EACH_ARRAY( WordIter, WordBuffer, unicode_t )
			{
				...
				// don't print chars that exceed the width of the text box
				// TODO should really be done in the Align method
				if ( !Bounded || ( Cursor.x + Max( 0, CharProp.m_A ) + CharProp.m_B + CharProp.m_C < BoundWidth ) )
				{
	...
	Align( Bounds, Flags, OutTypesetting, OutDimensions );
}
...
Mesh* Font::Print( const Array<unicode_t>& CodePoints, const SRect& Bounds, uint Flags ) const
{
	...
	Vector2 Dimensions;
	Arrange( CodePoints, Bounds, Flags, TypesetGlyphs, Dimensions );
	return Render( TypesetGlyphs, Dimensions, Bounds );
}

Mesh* Font::Render( Array<STypesetGlyph>& TypesetGlyphs, Vector2& Dimensions, const SRect& Bounds ) const
{
	// Cache the locale so we don't do multiple lookups while typesetting.
	const SFontLocale& Locale = GetCurrentLocale();
