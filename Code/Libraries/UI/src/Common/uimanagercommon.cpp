...
/*virtual*/ void UIManagerCommon::Initialize()
{
	...
	AddScreen( "SelectFileDialog",	new UIScreenSelectFileDialog );
}
...
void UIManagerCommon::ShowSelectFileDialog(
	bool				PauseGame,
...
{
	STATIC_HASHED_STRING( SelectFileDialog );
	UIScreenSelectFileDialog* const pSelectFileDialog = GetScreen<UIScreenSelectFileDialog> ( sSelectFileDialog );
	ASSERT( pSelectFileDialog );
	pSelectFileDialog->SetParameters(
		PauseGame,
		TitleString,
		TitleDynamicString,
		Path,
	...
	// TODO should be something like IsFile
	if ( FileUtil::IsFile( Path.CStr() ) )
	{
		pSelectFileDialog->SetFile( Path );
	}

	m_UIStack->Push( pSelectFileDialog );
}
