...
void UIScreen::InitializeFromDefinition( const SimpleString& DefinitionName )
{
	...
	// add widgets added by other widgets
	FOR_EACH_ARRAY( pWidget, m_DeferredWidgets, UIWidget* )
	{
		AddWidget( *pWidget );
	}
	m_DeferredWidgets.Clear();
...
void UIScreen::DeferredAddWidget( UIWidget* pNewWidget )
{
	m_DeferredWidgets.PushBack( pNewWidget );
}
