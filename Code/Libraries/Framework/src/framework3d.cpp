...
void Framework3D::Main()
{
	...
	catch ( RuntimeError& TheError )
	{
		PRINTF("%s\n", TheError.Message() );
		ShutDown();
		// hard shutdown. Don't invoke C++ magic cleanup stuff as some static
		// objects may be in an undefined state.
		_Exit(1);
	}
	ShutDown();
}
...
// show console only in debug mode!
	...
	ASSERT( pIconImage );
	const Surface		IconSurface	= Surface( ModManager::OpenStream( pIconImage ), Surface::ESFT_BMP );
...
void Framework3D::CreateSplashWindow( const uint WindowIcon, const char* const Title )
{
	...
	const Surface		SplashSurface		= Surface( ModManager::OpenStream( pSplashImage ), Surface::ESFT_BMP );
...
/*virtual*/ bool Framework3D::TickSDLEvents()
{
	...
	while( SDL_PollEvent( &Event ) )
	{
		...
		if (Event.type == SDL_TEXTINPUT && m_Keyboard->m_WantsTextInput )
		{
...
{
#if BUILD_MAC
	SimpleString ScreenshotPath = ObjCJunk::GetUserDirectory() + "/Screens";
#else
	SimpleString ScreenshotPath = "Screens";
#endif

	if( !FileUtil::PathExists( ScreenshotPath.CStr() ) )
	{
		FileUtil::MakePath( ScreenshotPath.CStr() );
	}

	const SimpleString TimeDateString = TimeDate::GetTimeDateString();
	const SimpleString ScreenshotName = SimpleString::PrintF( ScreenshotPath + "/%s.png", TimeDateString.CStr() );
...
{
	Keyboard *pKeyboard = GetInstance()->m_Keyboard;
	...
	case WM_CHAR:
		{
			if ( pKeyboard->m_WantsTextInput && wParam >= ' ' )
			{
				char Utf8Buffer[5];
				int Utf8Len = mini_utf8_encode( (int)wParam, Utf8Buffer, 5 );
				if ( Utf8Len > 0 )
				{
					Utf8Buffer[ Utf8Len ] = '\0';
					pKeyboard->AddText( Utf8Buffer );
				}
			}
		}
	}
