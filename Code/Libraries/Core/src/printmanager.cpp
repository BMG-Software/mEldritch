...
void PrintManager::Printf( const HashedString& Category, int Level, const char* Format, ... )
{
	...
	VPrintf( Category, Level, Format, Args );
	va_end( Args );
}

void PrintManager::VPrintf( const HashedString& Category, int Level, const char* Format, va_list Args )
{
...
int PrintManager::LastOutputLength()
{
	return strlen( m_StringBuffer );
}
