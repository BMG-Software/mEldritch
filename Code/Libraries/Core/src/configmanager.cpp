...
{
	m_Vars = &m_Environment.PushBack( Vars( m_UsingAllocator ? &m_Allocator : NULL ) );
	//m_Vars = &m_Environment[ 0 ];
}

ConfigManager::~ConfigManager()
{
	//m_Vars.Clear();
	FOR_EACH_ARRAY( EnvIter, m_Environment, Vars )
	{
		( *EnvIter ).Clear();
	}
	m_Environment.Clear();
...
{
	ConfigVar* pConfigVar = NULL;
	uint EnvSize = m_Environment.Size();

	// search environment stack until we find the config var
	for ( int i = EnvSize - 1; !pConfigVar && (  i >= 0 ); --i )
	{
		const Map< HashedString, VarMap >& Vars = m_Environment[ i ];
		const Map< HashedString, VarMap >::Iterator VarMapIter = Vars.Search( HASH_STRING( Context ) );
...
/* static */ void ConfigManager::PushEnvironment()
{
	GetInstance()->m_Vars = &GetInstance()->m_Environment.PushBack( Vars( m_UsingAllocator ? &m_Allocator : NULL ) );
}

/* static */ void ConfigManager::MergeEnvironment()
{
	Array< Vars > &Environment = GetInstance()->m_Environment;
	uint EnvSize = Environment.Size();
	if ( EnvSize > 1 )
	{
		Vars &MergeFrom = Environment[ EnvSize - 1 ];
		Vars &MergeTo 	= Environment[ EnvSize - 2 ];

		FOR_EACH_MAP( ContextIter, MergeFrom, HashedString, VarMap )
		{
			HashedString Context = ContextIter.GetKey();
			FOR_EACH_MAP( MergeIter, ContextIter.GetValue(), HashedString, ConfigVar )
			{
				MergeTo[ Context ][ MergeIter.GetKey() ] = MergeIter.GetValue();
			}
		}
...
/* static */ void ConfigManager::PopEnvironment()
{
	Array< Vars > &Environment = GetInstance()->m_Environment;
	uint EnvSize = Environment.Size();
	if ( EnvSize > 1 )
	{
		Environment.PopBack();
		GetInstance()->m_Vars = &Environment[ Environment.Size() - 1 ];
	}
}

/* static */ void ConfigManager::RemoveContext( const STRING_TYPE& Context )
{
	if ( Context != EmptyContext || GetInstance()->m_Environment.Size() > 1 )
	{
		GetInstance()->m_Vars->Remove( Context );
	}
}

#if BUILD_DEBUG
/* static */ void ConfigManager::Query( const char* Name, const char* Context )
{
	if ( !Context )
	{
		Context = "";
	...
	const ConfigVar* Result = GetInstance()->GetConfigVarForRead( sName, sContext );
