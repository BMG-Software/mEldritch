...
public:
	...
	void			WantTextInput( bool Want );
	void			AddText( const char* Text );
	bool			GetText( SimpleString& Text );
	void			ClearText();

	SKeyboardState	m_CurrentState;
	SKeyboardState	m_LastState;
	bool			m_WantsTextInput;
	
	// we use a fixed size buffer so that it will not eat up all memory
	// if we somehow forget to disable m_WantsTextInput
	char			m_TextBuffer[ TEXTBUFFER_SIZE ];
	bool			m_TextBufferFull;
};
