...
public:
	...
	// shrink array to range [Start, End], both inclusive
	void SliceToRange( uint Start, uint End )
	{
		if ( End < Start || Start >= m_Size )
		{
			return;
		}
		if ( End >= m_Size )
		...
		for( uint i = End + 1; i < m_Size; ++i )
		{
			m_Array[i].~C();
		}
		uint NewSize = End - Start + 1;
		memcpy( m_Array, &m_Array[Start], sizeof(C) * NewSize );
		m_Size = NewSize;
		uint NewCap = m_Capacity;
		do
		{
			NewCap = NewCap >> 1;
		}
