...
SDL_Keycode		SDLKeycodes[ Keyboard::EB_Max ] =
{
	...
	SDLK_UNKNOWN,	// nonetheless, for the level editor, these are filled in Keyboard::Tick
	SDLK_UNKNOWN,
};
#endif // BUILD_SDL
...
void Keyboard::Tick( float DeltaTime )
{
	...
	int x, y;
	const Uint32 SDLMouseState = SDL_GetMouseState(&x, &y);
	if (SDLMouseState & SDL_BUTTON(SDL_BUTTON_LEFT)) {
			m_CurrentState.m_Buttons[ EB_Mouse_Left ] = 1;
	} else if (SDLMouseState & SDL_BUTTON(SDL_BUTTON_MIDDLE)) {
		m_CurrentState.m_Buttons[ EB_Mouse_Middle ] = 1;
	} else if (SDLMouseState & SDL_BUTTON(SDL_BUTTON_RIGHT)) {
		m_CurrentState.m_Buttons[ EB_Mouse_Right ] = 1;
	}
#endif
}
...
void Keyboard::WantTextInput( bool Want )
{
	m_WantsTextInput = Want;
#if BUILD_SDL
	if ( Want )
	{
		SDL_StartTextInput();
...
void Keyboard::AddText( const char* Text )
{
	if ( Text && *Text && m_WantsTextInput && !m_TextBufferFull )
	{
		int TextBufferLen = strlen(m_TextBuffer);
		if ( TextBufferLen + strlen(Text) + 1 > TEXTBUFFER_SIZE )
		{
			m_TextBufferFull = true;
		}
		else
		{
			while ( *Text )
			{
				m_TextBuffer[ TextBufferLen++ ] = *Text++;
			}
			m_TextBuffer[ TextBufferLen ] = '\0';
		}
	}
}

bool Keyboard::GetText( SimpleString& Text )
{
	if ( !m_WantsTextInput || m_TextBuffer[ 0 ] == '\0' )
	{
		return false;
	}
	Text = m_TextBuffer;
...
{
	memset( m_TextBuffer, 0, TEXTBUFFER_SIZE );
	m_TextBufferFull = false;
}
