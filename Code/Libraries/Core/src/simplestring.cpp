...
,	m_Length( 0 )
{
	...
	InitializeNonTerminated( String.GetData(), String.Size() );
}
...
bool SimpleString::operator<( const SimpleString& String ) const
{
	if ( !m_String || !String.m_String )
	{
		return m_String < String.m_String ? -1 : m_String > String.m_String ? 1 : 0;
	}
	return ( 0 > strcmp( m_String, String.m_String ) );
}
...
bool SimpleString::StrILt( const SimpleString& String ) const
{
	if ( !m_String || !String.m_String )
	{
		return m_String < String.m_String ? -1 : m_String > String.m_String ? 1 : 0;
...
SimpleString SimpleString::Replace( uint Index, const char* const Replace ) const
{
	DEVASSERT( Replace );
	...
	NewStringBuffer.Resize( Index + ReplaceLength + 1 );

	memcpy(NewStringBuffer.GetData() + Index, Replace, ReplaceLength + 1);
...
/*static*/ SimpleString	SimpleString::PrintF( const SimpleString FormatString, ... )
{
	va_list Args;
	va_start( Args, FormatString );

	int Length = VSPRINTF_COUNT( FormatString.CStr(), Args ) + 1;
	char* pBuffer = Allocate( Length );
	VSPRINTF( pBuffer, Length, FormatString.CStr(), Args );
