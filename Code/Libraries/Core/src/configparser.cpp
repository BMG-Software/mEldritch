...
void ConfigParser::Parse( const IDataStream& Stream )
{
	...
	for(;;)
	{
		...
		else if ( c == '?' ) // ?? compile time only
		{
			if( Token.m_TokenType == SToken::ET_None && Tokens.Size() >= 2 && Tokens[Tokens.Size() - 1].m_TokenType == SToken::ET_Equals && Tokens[Tokens.Size() - 2].m_TokenType == SToken::ET_Name)
			{
				// Retrieve value for counter from environment
				SimpleString CounterName = Tokens[Tokens.Size() - 2].m_TokenString.GetData();

				DEBUGCATPRINTF( "Core", 2, "retrieving counter value %s.%s\n", Context.CStr(), CounterName.CStr() );

				int CounterValue = -1;
				
				// attempt to retrieve from Tokens array
				DEBUGCATPRINTF( "Core", 2, "retrieving from token stream\n" );
				List<int>::Iterator MyCounters = ArrayCounters.Begin();
				FOR_EACH_ARRAY(Tok, Tokens, SToken) {
					if ((*Tok).m_TokenType == SToken::ET_Name && CounterName == (*Tok).m_TokenString && MyCounters.IsValid()) {
						// TODO should check for '=' and Counter tokens
						CounterValue = *MyCounters;
					} else if ((*Tok).m_TokenType == SToken::ET_Counter) {
						MyCounters++;
						if (!MyCounters.IsValid())
						{
							break;
						}
					}
				...
				if ( CounterValue == -1 )
				{
					DEBUGCATPRINTF( "Core", 2, "retrieving from token stream failed, using environment\n" );
					CounterValue = ConfigManager::GetInt( CounterName, 0, Context ) - 1;
				}
				DEBUGCATPRINTF( "Core", 2, "retrieved counter value is %d\n", CounterValue );
				ArrayCounters.PushBack( CounterValue );
				Token.m_TokenType = SToken::ET_Counter;
			}
			else if( Token.m_TokenType == SToken::ET_String )
			{
