...
namespace FileUtil
{
	...
	void			GetAllFiles( const char* Path, const char* Extension, Array< SimpleString >& OutFiles );
	...
	const char*		StripAllButExtension( const char* Path );	// returns the extension. (returns substring)
	SimpleString	StripFilename( const char* Path );			// Strips everything after and inclusing the last '/' (returns new string)
